...
```{r}
exp_test <- read.csv("data/experimenter.test.csv")

#tapply(exp_test$Percent_correct, exp_test$Key_Scheme, sd)

#https://towardsdatascience.com/understanding-auc-roc-curve-68b2303cc9c5
tapply(exp_test$Percent_correct, exp_test$Key_Scheme, std.error)
```
...
After choosing a cost matrix of [0 0 4 0] for false negatives Randomforest performs significant lower in comparison with the default settings. 
...
Randomforest accuracy increases 0.05 percent with cost matrix of [0 0 2 0] and lowering the false negatives with 19 instances. The accuracy decrease around 1.3% when using the cost matrix [0 0 3 0] and still maintaining an accuracy of 85.24%. The false negatives decrease with 6 instances but the false positives increases with 15 instances. Using the cost matrix of [0 0 3 0] decreases false negatives a little more than 50% in comparison with the default matrix. It's more viable to lower the false negatives instead of increasing the false positives. AdaBoostm1 performs worse in accuracy and the amount of false negatives in comparison with Randomforest with a cost matrix of [0 0 3 0]. That's why the final model Randomforest with cost matrix of [0 0 3 0] will be selected.

Weka scheme: **meta.CostSensitiveClassifier '-cost-matrix \"[0.0 1.0; 3.0 0.0]\" -S 1 -W trees.RandomForest -- -P 100 -I 100 -num-slots 1 -K 0 -M 1.0 -V 0.001 -S 1' -110658209263002404**
With an accuracy of 85.24%
